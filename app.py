...
if uploaded_files:
    ...
    if n1 is not None:
        col1.metric(
    "CA HT",
    safe_format(latest["CA_HT"], " €"),
    safe_format(variation(latest["CA_HT"], n1["CA_HT"]) if n1 else None, "%")
)
...
col4.metric(
    "Marge",
    safe_format(latest["Marge"], " %"),
    safe_format(variation(latest["Marge"], n1["Marge"]) if n1 else None, "%")
)
    else:
